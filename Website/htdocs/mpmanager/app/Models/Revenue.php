...
class Revenue extends Model
{
    ...
    public function registeredMetersForMiniGridByConnectionGroupTill($miniGridId, $connectionId, $endDate)
    {
        $sql = 'SELECT COUNT(meter_parameters.id) as registered_connections from meter_parameters ' .
            ...
            'and addresses.city_id IN (SELECT id from cities where mini_grid_id = ' . $miniGridId . ') and addresses.owner_type = "meter_parameter" ';
    ...
    public function registeredMetersForClusterByConnectionGroupTill($clusterId, $connectionId, $endDate)
    {
        $sql = 'SELECT COUNT(meter_parameters.id) as registered_connections from meter_parameters ' .
            ...
            ' where meter_parameters.connection_group_id=:connection_id and' .
            ' DATE(meter_parameters.created_at) <= DATE(:endDate) ' .
            'and addresses.city_id IN (SELECT city_id from cities where cluster_id = ' . $clusterId . ' )  and addresses.owner_type = "meter_parameter" ';
        $sth = DB::connection()->getPdo()->prepare($sql);
        $sth->bindValue(':connection_id', $connectionId, PDO::PARAM_INT);
        $sth->bindValue(':endDate', $endDate, PDO::PARAM_STR);
    ...
    {
        //get meters which are registered in the given period
        $sql = 'SELECT COUNT(meters.serial_number) as registered_connections, connection_groups.name,YEARWEEK(meter_parameters.created_at,3) as period from meters' .
            ' LEFT JOIN meter_parameters on meter_parameters.meter_id = meters.id ' .
            'left join addresses on addresses.owner_id = meter_parameters.id ' .
            ...
            'and addresses.city_id IN (SELECT city_id from cities where cluster_id = :clusterId )  and addresses.owner_type = "mini-grid" ';
    ...
    public function dailyTariffBalanceForPeriod(int $tariffId, string $startDate, string $endDate)
    {
        ...
        $sql = 'SELECT sum(transactions.amount) as total, DATE(transactions.created_at) as result_date FROM transactions' .
            ' LEFT JOIN vodacom_transactions on vodacom_transactions.id = transactions.original_transaction_id' .
            ' AND transactions.original_transaction_type ="vodacom_transaction"' .
            ' LEFT JOIN airtel_transactions on airtel_transactions.id = transactions.original_transaction_id' .
            ' AND transactions.original_transaction_type ="airtel_transaction"' .
    ...
    {
        /*$sql = 'SELECT sum(transactions.amount) as total, ' . $periodGroup . ' as result_date, (SELECT name from connection_types WHERE id  =:connectionSelect LIMIT 1 ) as connection FROM transactions' .
